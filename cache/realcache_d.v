...
output reg [31:0] axib_wr_data;

//input from RAM for write
input wire axib_wr_rdy;

//internal signals
reg [52-1:0] cache_ram [4096-1:0][`CC_WAY_SIZE-1:0];
...
reg [12-1:0] buffer_cpu_addr_set;
...
reg [31:0] buffer_cache_replace_line;
// reg [52-1:0] buffer_axi_load_line;
...
always @(posedge clk) begin
    if(!rst_n) begin
        ...
        buffer_cache_replace_line <= 32'b0;
    end
...
always @(*) begin
    if(!rst_n) begin
        ...
        if(cache_last_state == `CC_STATE_AXILOADING_LAST
            ...
            output_rdata <= cache_ram[buffer_cpu_addr_set][buffer_cache_way_to_load][31:0];
        end
        else if(cache_last_state == `CC_STATE_AVAILABLE
            && buffer_cpu_rw_op == `CC_CPU_OP_RD
            && buffer_cpu_rw_en) begin
            output_rdata_valid <= 1;
            output_rdata <= cache_ram[buffer_cpu_addr_set][buffer_cache_way_hit][31:0];
            end
        else begin
            output_rdata_valid <= 0;
            output_rdata <= 32'b0;
    ...
    if(!rst_n)begin
        ...
        if(cache_state == `CC_STATE_AXIREADING && axib_rd_rdy) begin
            ...
            axib_rd_addr <= (cache_last_state==`CC_STATE_AXISTALL_READ) ? 
                            {buffer_cpu_rw_addr[31:2], 2'b0} :
                            {cpu_rw_addr[31:2], 2'b0};
        end
        else begin
            axib_rd_req <= 0;
            axib_rd_type <= 3'b000;
...
always @(*) begin
    if(!rst_n) begin
        ...
        if(cpu_addr_tag == cache_ram[cpu_addr_set][0][51:34] 
            && cache_ram[cpu_addr_set][0][33]) begin
            cache_hit <= 1;
            cache_hit_way <= 2'd0;
        end
        else if(cpu_addr_tag == cache_ram[cpu_addr_set][1][51:34] 
            && cache_ram[cpu_addr_set][1][33]) begin
            cache_hit <= 1;
            cache_hit_way <= 2'd1;
        end
        else if(cpu_addr_tag == cache_ram[cpu_addr_set][2][51:34] 
            && cache_ram[cpu_addr_set][2][33]) begin
            cache_hit <= 1;
            cache_hit_way <= 2'd2;
        end
        else if(cpu_addr_tag == cache_ram[cpu_addr_set][3][51:34] 
            && cache_ram[cpu_addr_set][3][33]) begin
            cache_hit <= 1;
            cache_hit_way <= 2'd3;
        end
        else begin
...
always @(*) begin
    if(!rst_n)begin
        ...
        // cache_way_full <= cache_ram[cpu_addr_set][0][33] 
        //     && cache_ram[cpu_addr_set][1][33] 
        //     && cache_ram[cpu_addr_set][2][33] 
        //     && cache_ram[cpu_addr_set][3][33];
        if (!cache_ram[cpu_addr_set][0][33]) begin
            cache_way_full <= 0;
            cache_way_to_load <= 2'd0;
        end
        else if (!cache_ram[cpu_addr_set][1][33]) begin
            cache_way_full <= 0;
            cache_way_to_load <= 2'd1;
        end
        else if (!cache_ram[cpu_addr_set][2][33]) begin
            cache_way_full <= 0;
            cache_way_to_load <= 2'd2;
        end
        else if (!cache_ram[cpu_addr_set][3][33]) begin
            cache_way_full <= 0;
...
//cache_replace_dirty
assign cache_replace_dirty = cache_ram[cpu_addr_set][cache_way_to_replace][32];

//cache_replace_addr
always @(*) begin
    cache_replace_addr <= {cache_ram[cpu_addr_set][cache_way_to_replace][51:34], cpu_addr_set, 2'b0};
    cache_replace_line <= {cache_ram[cpu_addr_set][cache_way_to_replace][31:0]};
end
...
always @(posedge clk) begin
    if(!rst_n) begin
        ...
        cache_ram[buffer_cpu_addr_set][buffer_cache_way_to_load] <= {buffer_cpu_addr_tag, 1'b1,buffer_cpu_rw_op==`CC_CPU_OP_WR, ram_wdata};
    end
end
